...
java -jar %PARSER_HOME%\bin\target\urtlog-1.0.jar -l %PARSER_HOME%\logs\%LOG_NAME% -o %JSON_HOME%\%JSON_NAME% -lim 15 -types 7 -admin PanzerjagerTiger -exclude "Bonev|OHAD|kobii|deadwomanwalking|Shalom|Kilaka|RODOV.*?"
cd %JSON_HOME%
git add %JSON_NAME% 
